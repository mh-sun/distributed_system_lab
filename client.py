...
def send_rating(r_name, d_name):
    ...
    requests.post(host_port + "rate", json=rate_req)
...
# sio.emit("test", {"test": "testData"}, namespace='/chat')
...
while True:
    ...
    requests.post(host_port + "rider", json=rider_req)
    requests.post(host_port + "driver", json=driver_req)
